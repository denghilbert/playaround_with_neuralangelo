...
class Model(BaseModel):
    ...
    def build_model(self, cfg_model, cfg_data):
        ...
        if cfg_model.appear_embed.enabled:
            ...
            # use SH as illumination representation
            self.env_params = nn.ParameterList(
                [nn.Parameter(torch.tensor([
                    ...
                    # [6.5793e-02, 4.3270e-02, -1.7002e-01],
                    # [-7.2674e-02, 4.5177e-02, 2.2858e-01]
                ], dtype=torch.float32)) for x in range(cfg_data.num_images)])
        else:
        ...
        dim = 256 + 9
        for i in range(1):
            shadow_layers.append(nn.Linear(dim, 128))
    ...
    def render_rays(self, center, ray_unit, sample_idx=None, stratified=False):
        ...
        app, app_outside, fg_sph = self.get_appearance_embedding(sample_idx, ray_unit.shape[1])
        output_object = self.render_rays_object(center, ray_unit, near, far, outside, app, fg_sph, stratified=stratified)
        ...
        # Calculate weighted shadow map
        shadow = output_object['shadow_map']
        fg_weight = render.alpha_compositing_weights(output_object["alphas"])
        shadow_map = torch.sum(shadow * fg_weight, dim=-2)
        rgb = rgb * shadow_map
    ...
    def render_rays_object(self, center, ray_unit, near, far, outside, app, fg_sph, stratified=False):
        with torch.no_grad():
        ...
        # Add shadow layers for Spherical Harmonics
        fg_sph = fg_sph.unsqueeze(0).unsqueeze(0).unsqueeze(0).expand(feats.shape[:-1] + fg_sph.shape)
        shadow_input = torch.cat((feats, fg_sph), dim=-1)
        shadow_map = self.shadow_layers(shadow_input)
        # Collect output.
        output = dict(
            rgbs=rgbs,  # [B,R,N,3]
            sdfs=sdfs[..., 0],  # [B,R,N]
